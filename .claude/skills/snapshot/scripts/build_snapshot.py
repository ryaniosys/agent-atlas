...
def render_header(month: str, totals: dict, content: dict) -> str:
    org = content["meta"].get("org", "")
    subtitle = content["meta"].get("subtitle", "Agent Ecosystem")
    meta_line = (
...
<p class="subtitle">{org} &mdash; {subtitle} &mdash; {month}<br>
{meta_line}</p>
"""
...
def render_footer(month: str, content: dict) -> str:
    org = content["meta"].get("org", "")
    return f"""
<div style="margin-top: 40px; padding-top: 16px; border-top: 1px solid #30363d; color: #555; font-size: 8px;">
  Generated {month} &middot; {org} &middot; Multi-Agent Architecture Reference v1.1
</div>
"""
...
def build_html(scan: dict, content: dict, month: str) -> str:
    ...
    sections = [
        render_header(month, scan["totals"], content),
...
<head>
<meta charset="UTF-8">
<title>Multi-Agent Architecture Overview</title>
<style>
{CSS}
</style>
</head>
